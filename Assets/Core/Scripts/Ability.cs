...
public class Ability : ScriptableObject, IVisualCodeHandler
{
    ...
    public string abilityFlavourText;
    ...
    public const float MOVEMENT_DELAY_AFTER_CAST = 0.7f;

    public string abilityTag = "";

    public IVisualCodeHandler CopyGeneral(string name)
    {
        return Copy();
    }
    ...
    {
        return ((2.0f - (caster.stats.GetValue(Stat.ResourceCostReduction))) * caster.GetAbilityCostModifier(this)); //* caster.GetAbilityCostModifier(this)
    }
    ...
    /// </summary>
    public void StartCast (Unit caster, Unit target, Vector3 targetPosition, bool payCost = true, bool applyCastingRestrictions = true)
    {
        needsReminderFlash = false;
        ClampTargetLocation(caster, target, ref targetPosition);       
        if (payCost) ApplyResourceChange(caster);
        if (applyCastingRestrictions) PlayAbilityAnimation(caster);
        if (applyCastingRestrictions) ApplyCastingRestrictionsToCaster(caster);
    ...
    public void CancelCast (Unit caster)
    {
        caster.abilityBeingCast = null;
    }
    ...
    private void ApplyCastingRestrictionsToCaster (Unit caster)
    {
        ...
        if (hasSpecificCastTime)
        {
            ...
            if (castTime > 0 && !canMoveWhileCasting)
    ...
    private void SendCastBeginEvents (Unit caster, Unit target, Vector3 targetPosition)
    {
        Dictionary<string, object> presets = new Dictionary<string, object>
            {
                { VisualCodeLabels.Presets.Events.Units.PRESET_CASTING_UNIT, caster },
                { VisualCodeLabels.Presets.Events.Abilities.PRESET_ABILITY_CAST, this },
                { VisualCodeLabels.Presets.Events.Units.PRESET_TARGET_UNIT, target },
                { VisualCodeLabels.Presets.Events.Vectors.PRESET_TARGET_POSITION, targetPosition }
            };
        engine.TriggerEvent(presets, "WhenUnitBeginsCastingThisAbility");
        GameManager.events.OnAbilityCastStarted.Invoke(new GameEvents.OnAbilityCastStartedInfo(caster, this, target, targetPosition));
    }
    ...
    private void SendCastCompleteEvents (Unit caster, Unit target, Vector3 targetPosition)
    {
        ...
        GameManager.events.OnAbilityCastFinished.Invoke(new GameEvents.OnAbilityCastFinishedInfo(caster, this, target, targetPosition));
    }
    ...
    public Ability Copy ()
    {
        ...
        copy.engine.engineHandler = copy;
        return copy;
    }
    ...
    /// </summary>
    public void Unlock(bool reminderFlash)
    {
        isUnlocked = true;
        needsReminderFlash = reminderFlash;
    }

    public Vector3 GetClosestPositionInRange (Unit caster, Vector3 target, bool allowMeleeToChange = false)
    {
        float distance = Vector3.Distance(caster.transform.position, target);
        if (allowMeleeToChange && AbilityRequiresMelee())
        {
    ...
    public static bool AbilitiesShareTemplate(Ability ability1, Ability ability2)
    {
        if (ability1.template != null) ability1 = ability1.template;
        if (ability2.template != null) ability2 = ability2.template;
    ...
    public Texture2D GetIcon() => abilityIcon == null ? null : abilityIcon.texture;
