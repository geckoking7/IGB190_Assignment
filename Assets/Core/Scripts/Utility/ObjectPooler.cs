...
public class ObjectPooler : MonoBehaviour
{
    ...
    public static GameObject InstantiatePooled(GameObject prefab, Vector3 position, Quaternion rotation)
    {
        ...
        IPooledObject pooledObjectControls = obj.GetComponent<IPooledObject>();
        if (pooledObjectControls != null) pooledObjectControls.Setup();
    ...
    public static void DestroyPooled(GameObject objectToPool)
    {
        ...
        if (Instance.inUseObjects.TryGetValue(objectToPool, out GameObject prefab))
        {
            ...
            objectToPool.transform.SetParent(Instance.transform);
            ...
            IPooledObject pooledObjectControls = objectToPool.GetComponent<IPooledObject>();
            if (pooledObjectControls != null) pooledObjectControls.Cleanup();

            // Add the object back to the pool
            objectPool.Enqueue(objectToPool);

        }
        else if (!objectToPool.gameObject.activeSelf)
        {
            // Object has already been destroyed, don't destroy it again.
        }
        else
        {
