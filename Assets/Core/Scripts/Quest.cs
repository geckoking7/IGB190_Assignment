...
public class Quest
{
    ...
    public float questStartingTime;
    ...
    public QuestItem GetQuestRequirement (string requirement)
    {
        if (_questItems.ContainsKey(requirement))
            return _questItems[requirement];
        else
            return null;
    }
    ...
    public string GetMainRequirementText ()
    {
        if (_questItems.Count > 0)
            return _questItems.Keys.First();
        else
            return string.Empty;
    }

    public void AddCompletionRequirement(string description, int maxProgress = 1, int currentProgress = 0)
    {
        _questItems[string.Empty] = new QuestItem(string.Empty, description, maxProgress, currentProgress);
        HandleQuestUpdate();
    }
    ...
    {
        GetQuestItem(string.Empty).IncrementProgress(value);
        HandleQuestUpdate();
    }
    ...
    {
        GetQuestItem(string.Empty).DecrementProgress(value);
        HandleQuestUpdate();
    }
    ...
    {
        value = Mathf.Max(0, value); // Quest progress cannot be negative.
    ...
    {
        GetQuestItem(string.Empty).SetProgress(value);
        HandleQuestUpdate();
    }
    ...
    /// <summary>
    /// Complete the quest immediately, regardless of progress.
    /// </summary>
    ...
    {
        if (label == string.Empty) label = _questItems.Keys.First();
