...
public class StatusMessageUI : MonoBehaviour
{
    ...
    {
        if (GameManager.assets.statusMessageUI == null)
        {
            Debug.LogError("No prefab for the StatusMessage found (GameManager > Assets > StatusMessageUI). Unable to spawn one.");
            return;
        }
    ...
    private void UpdateVisual()
    {
        ...
        float scale = scaleCurve.Evaluate(elapsedRatio);
