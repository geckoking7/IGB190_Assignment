...
public class SetValueEditor : EditorWindow
{
    ...
    public static UnityEngine.Object[] valueOptions;
    ...
    public static SetValueEditor OpenWindow (EditorWindow parent, Vector2 position, GeneralNode node, IVisualCodeHandler engineHandler)
    {
        ...
        else if (node is InteractableNode) CacheValueOptions_Interactables();
        else if (node is EffectNode) CacheValueOptions_Feedback();
        else if (node is ItemSetNode) CacheValueOptions_ItemSets();
        else if (node is BuffNode) CacheValueOptions_Buffs();
        else if (node is AbilityNode) CacheValueOptions_Abilities();
        else if (node is ItemNode) CacheValueOptions_Items();

        allowValues = window.nodeCopy.GetAllowValue();
        allowPresets = window.nodeCopy.GetAllowPreset();
        allowFunctions = window.nodeCopy.GetAllowFunction();
        ...
        if (window.nodeCopy.returnType == GeneralNode.ReturnType.Temp && window.nodeCopy.GetAllowValue())
        {
            window.nodeCopy.returnType = GeneralNode.ReturnType.Value;
        }
        else if (window.nodeCopy.returnType == GeneralNode.ReturnType.Temp && window.nodeCopy.GetAllowPreset() && presetStrings.Length > 0)
        {
            window.nodeCopy.returnType = GeneralNode.ReturnType.Preset;
            window.nodeCopy.presetName = presetStrings[0];
        }
        else if (window.nodeCopy.returnType == GeneralNode.ReturnType.Temp && window.nodeCopy.GetAllowFunction())
        {
            window.nodeCopy.returnType = GeneralNode.ReturnType.Function;
            window.nodeCopy.functionDescription = functionNodes[0].Template.functionDescription;
            window.nodeCopy.functionDynamicDescription = functionNodes[0].Template.functionDynamicDescription;
            window.nodeCopy.functionName = functionNodes[0].Template.functionName;
            window.nodeCopy.functionEvaluators = functionNodes[0].Template.functionEvaluators;
        }
    ...
    private static void CacheFunctions (GeneralNode node)
    {
        ...
        {
            /*
            // Unit Group nodes are special and can allow single units as well.
            if (node.GetType() == typeof(UnitGroupNode))
            {
                if (current.GetType() == typeof(UnitGroupNode))
                {
                    functionNodesList.Add(current);
                    functionNodeStringsList.Add("Group of Units/" + current.functionDescription);
                    //functionNodeStringsList.Add(current.functionDescription);
                }
                else if (current.GetType() == typeof(UnitNode))
                {
                    functionNodesList.Add(current);
                    functionNodeStringsList.Add("Single Unit/" + current.functionDescription);
                    //functionNodeStringsList.Add(current.functionDescription);
                }
            }
            else */
            ...
            if (node.GetType() == typeof(VectorNode) && current.GetType() == typeof(UnitNode))
            {
                functionNodesList.Add(current);
                functionNodeStringsList.Add("Unit/" + current.functionDescription);
            }
            else if (node.GetType() == typeof(VectorNode) && current.GetType() == typeof(VectorNode))
            {
                functionNodesList.Add(current);
                functionNodeStringsList.Add("Location/" + current.functionDescription);
            }

            else if (node.GetType() == typeof(UnitGroupNode) && current.GetType() == typeof(UnitNode))
            {
                functionNodesList.Add(current);
                functionNodeStringsList.Add("Single Unit/" + current.functionDescription);
            }
            else if (node.GetType() == typeof(UnitGroupNode) && current.GetType() == typeof(UnitGroupNode))
            {
                functionNodesList.Add(current);
                functionNodeStringsList.Add("Group of Units/" + current.functionDescription);
            }
            else if (current.GetType() == node.GetType())
            {
                functionNodesList.Add(current);
                functionNodeStringsList.Add(current.functionDescription);
            }
    ...
    /// 
    /// </summary>
    private static void CachePresets (GeneralNode node, IVisualCodeHandler engineHandler)
    {
        ...
        VisualCodeScript script = engineHandler.GetEngine().selectedScript;
        ...
        {
            if (
                (node is UnitNode && VisualCodeLabels.Presets.Events.Units.UNIT_PRESETS.Contains(preset)) ||
                (node is NumberNode && VisualCodeLabels.Presets.Events.Numbers.NUMBER_PRESETS.Contains(preset)) ||
                (node is ProjectileNode && VisualCodeLabels.Presets.Events.Projectiles.PROJECTILE_PRESETS.Contains(preset)) ||
                (node is ItemNode && VisualCodeLabels.Presets.Events.Items.ITEM_PRESETS.Contains(preset)) ||
                (node is StringNode && VisualCodeLabels.Presets.Events.Strings.STRING_PRESETS.Contains(preset)) ||
                (node is AbilityNode && VisualCodeLabels.Presets.Events.Abilities.ABILITY_PRESETS.Contains(preset)) ||
                (node is VectorNode && VisualCodeLabels.Presets.Events.Vectors.VECTOR_PRESETS.Contains(preset)) ||
                (node is BoolNode && VisualCodeLabels.Presets.Events.Bools.BOOL_PRESETS.Contains(preset)) ||
                (node is InteractableNode && VisualCodeLabels.Presets.Events.Interactables.INTERACTABLE_PRESETS.Contains(preset)) ||
                (node is UnitGroupNode && VisualCodeLabels.Presets.Events.Units.UNIT_PRESETS.Contains(preset)) ||
                (node is VectorNode && VisualCodeLabels.Presets.Events.Units.UNIT_PRESETS.Contains(preset))
                )
            {
        ...
        if (node is UnitNode || node is UnitGroupNode)
        {
            presetNodeStringsList.Add(VisualCodeLabels.Presets.Events.Dynamic.PRESET_UNIT_PLAYER);
            presetNodeStringsList.Add(VisualCodeLabels.Presets.Events.Dynamic.PRESET_UNIT_LAST_CREATED);
        }
        if (node is ProjectileNode)
        {
            presetNodeStringsList.Add(VisualCodeLabels.Presets.Events.Dynamic.PRESET_PROJECTILE_LAST_CREATED);
        }
        if (node is UnitGroupNode)
        {
            presetNodeStringsList.Add(VisualCodeLabels.Presets.Events.Dynamic.PRESET_ALL_ENEMIES);
            presetNodeStringsList.Add(VisualCodeLabels.Presets.Events.Dynamic.PRESET_ALL_ALLIES);
            presetNodeStringsList.Add(VisualCodeLabels.Presets.Events.Dynamic.PRESET_ALL_NONPLAYER_ALLIES);
            presetNodeStringsList.Add(VisualCodeLabels.Presets.Events.Dynamic.PRESET_ALL_UNITS);
            presetNodeStringsList.Add(VisualCodeLabels.Presets.Events.Dynamic.PRESET_ALL_MONSTERS);
        }
        if (parentWindow is AbilityEditor)
        {
            if (node is UnitNode || node is UnitGroupNode) 
            {
                presetNodeStringsList.Add(VisualCodeLabels.Presets.Events.Dynamic.PRESET_ABILITY_OWNER);
            }
            else if (node is AbilityNode)
            {
                presetNodeStringsList.Add(VisualCodeLabels.Presets.Events.Dynamic.PRESET_ABILITY_THIS);
            }
        }
        else if (parentWindow is ItemEditor)
        {
            if (node is UnitNode || node is UnitGroupNode)
            {
                presetNodeStringsList.Add(VisualCodeLabels.Presets.Events.Dynamic.PRESET_ITEM_OWNER);
            }
            else if (node is ItemNode)
            {
                presetNodeStringsList.Add(VisualCodeLabels.Presets.Events.Dynamic.PRESET_ITEM_THIS);
            }
        }
        else if (parentWindow is BuffEditor)
        {
            if (node is BuffNode)
            {
                presetNodeStringsList.Add(VisualCodeLabels.Presets.Events.Dynamic.PRESET_BUFF_THIS);
            }
            else if (node is UnitNode || node is UnitGroupNode)
            {
                presetNodeStringsList.Add(VisualCodeLabels.Presets.Events.Dynamic.PRESET_UNIT_BUFF_APPLIER);
                presetNodeStringsList.Add(VisualCodeLabels.Presets.Events.Dynamic.PRESET_UNIT_BUFF_OWNER); 
            }
            else if (node is NumberNode)
            {
                presetNodeStringsList.Add(VisualCodeLabels.Presets.Events.Dynamic.PRESET_UNIT_BUFF_STACKS);
            }
        }
    ...
    public void Submit ()
    {
        ...
        if (parentWindow != null)
        {
            ...
            parentWindow.Focus();
            LogicEngineEditor.hoveredNode = null;
            LogicEngineEditor.hoveredNodeDepth = -1;
        }
    ...
    private static void CacheValueOptions_ItemSets()
    {
        List<UnityEngine.Object> components = new List<UnityEngine.Object>();
        List<string> componentNames = new List<string>();
        components.Add(null);
        componentNames.Add("No Set");
        ...
        foreach (ItemSet itemSet in itemSets)
        {
            if (itemSet != null)
            {
                componentNames.Add(itemSet.name);
                components.Add(itemSet);
            }
        }
    ...
    private static void CacheValueOptions_Projectiles()
    {
        List<MonoBehaviour> components = new List<MonoBehaviour>();
        List<string> componentNames = new List<string>();
        string[] interactionGUIDs = AssetDatabase.FindAssets("t:prefab", new[] { VisualCodeLabels.Folders.ASSETS });
        foreach (string guid in interactionGUIDs)
        {
            GameObject asset = AssetDatabase.LoadMainAssetAtPath(AssetDatabase.GUIDToAssetPath(guid)) as GameObject;
    ...
    private static void CacheValueOptions_Interactables()
    {
        List<MonoBehaviour> components = new List<MonoBehaviour>();
        List<string> componentNames = new List<string>();
        string[] interactionGUIDs = AssetDatabase.FindAssets("t:prefab", new[] { VisualCodeLabels.Folders.ASSETS });
        ...
        {
            GameObject asset = AssetDatabase.LoadMainAssetAtPath(AssetDatabase.GUIDToAssetPath(guid)) as GameObject;
            CustomInteractable component = asset.GetComponent<CustomInteractable>();
            if (component != null)
            {
                components.Add(component);
    ...
    public static void CacheValueOptions_Buffs()
    {
        Buff[] buffs = Resources.LoadAll<Buff>(VisualCodeLabels.Folders.BUFFS);
        string[] buffNames = new string[buffs.Length];
        for (int i = 0; i < buffNames.Length; i++)
        {
            buffNames[i] = buffs[i].name;
        }
        valueOptionStrings = buffNames;
        valueOptions = buffs;
    }

    public static void CacheValueOptions_Abilities()
    {
        Ability[] abilities = Resources.LoadAll<Ability>(VisualCodeLabels.Folders.ABILITIES);
        string[] abilityNames = new string[abilities.Length];
        for (int i = 0; i < abilityNames.Length; i++)
        {
            abilityNames[i] = abilities[i].name;
        }
        valueOptionStrings = abilityNames;
        valueOptions = abilities;
    }

    public static void CacheValueOptions_Items()
    {
        Item[] items = Resources.LoadAll<Item>(VisualCodeLabels.Folders.ITEMS);
        string[] itemNames = new string[items.Length];
        for (int i = 0; i < itemNames.Length; i++)
        {
            itemNames[i] = items[i].name;
        }
        valueOptionStrings = itemNames;
        valueOptions = items;
    }
    ...
    private float DrawValuePanel(Rect rect)
    {
        ...
        //else if (nodeCopy is BuffNode)
        //    nodeCopy.SetValue(EditorGUI.ObjectField(valueRect, (Buff)nodeCopy.GetValue(), typeof(Buff), false));
    ...
    private float DrawFunctionPanel (Rect rect)
    {
        ...
        int index = -1;
        //if (nodeCopy.Template != null)
        //    index = Mathf.Max(0, Array.IndexOf(functionNodeStrings, nodeCopy.Template.functionDescription));
        //
        if (nodeCopy.Template != null)
        {
            index = Mathf.Max(0, Array.IndexOf(functionNodes, nodeCopy.Template));
        }
        ...
        //int index = Mathf.Max(0, Array.IndexOf(functionNodeStrings, nodeCopy.Template.functionDescription));
        Rect valueRect = new Rect(rect.x + 90, rect.y, rect.width - rect.x - 95, 20);
        ...
        string label = index > 0 ? functionNodeStrings[index] : "";
        if (GUI.Button(valueRect, label, EditorStyles.popup))
        {
            MyListSearchProvider searchProvider = CreateInstance<MyListSearchProvider>();
            searchProvider.Setup(functionNodeStrings, (x) => {
                if (x < 0) x = 0;
                nodeCopy.functionDescription = functionNodes[x].Template.functionDescription;
                nodeCopy.functionDynamicDescription = functionNodes[x].Template.functionDynamicDescription;
                nodeCopy.functionName = functionNodes[x].functionName;

                nodeCopy.functionEvaluators = new GeneralNode[functionNodes[x].functionEvaluators.Length];
                for (int i = 0; i < functionNodes[x].functionEvaluators.Length; i++)
                {
                    nodeCopy.functionEvaluators[i] = functionNodes[x].functionEvaluators[i].Copy();
                }
                nodeCopy.returnType = GeneralNode.ReturnType.Function;
            });
            SearchWindow.Open(new SearchWindowContext(new Vector2(valueRect.width / 2.0f + valueRect.x + position.x, position.y + valueRect.y + 35), valueRect.width, 0), searchProvider);
        }
        ...
        {
            if (newID < 0) newID = 0;
            nodeCopy.functionDescription = functionNodes[newID].Template.functionDescription;
            nodeCopy.functionDynamicDescription = functionNodes[newID].Template.functionDynamicDescription;
