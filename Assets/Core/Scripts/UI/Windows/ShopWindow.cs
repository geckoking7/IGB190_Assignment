...
public class ShopWindow : UIWindow
{
    ...
    {
        float amount = Mathf.Round(item.itemCost * 0.5f);
        GameManager.player.AddGold(amount);
        sellInventory.RemoveItem(item);
        GameManager.events.OnItemSold.Invoke(new GameEvents.OnItemSoldInfo(item, amount));
    ...
    private void CreateMarketplaceItems(Item.ItemType filter)
    {
        ...
        {
            if (item.canPurchaseInShop && (item.classRequirement == "" || item.classRequirement == GameManager.player.unitName))
            {
    ...
    public void PurchaseItem(Item item)
    {
        ...
        purchaseButton.gameObject.SetActive(false);
        GameManager.events.OnItemPurchased.Invoke(new GameEvents.OnItemPurchasedInfo(item, item.itemCost));
    }
