...
public class StatModifier
{
    ...
    public void AddTimedModifier (Modifier modifier)
    {
    ...
    public void SetStackCount (string label, int newStacks)
    {
        Modifier modifier = GetModifierWithLabel(label, timedModifiers);
        if (modifier != null) modifier.Stacks = newStacks;
        modifier = GetModifierWithLabel(label, permanentModifiers);
        if (modifier != null) modifier.Stacks = newStacks;
        requiresUpdate = true;
    }

    public void SetDuration(string label, float duration)
    {
        Modifier modifier = GetModifierWithLabel(label, timedModifiers);
        if (modifier != null) modifier.RemoveAt = Time.time + duration;
        modifier = GetModifierWithLabel(label, permanentModifiers);
        if (modifier != null) modifier.RemoveAt = Time.time + duration;
        requiresUpdate = true;
    }
